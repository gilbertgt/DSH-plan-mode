...
The lockfile is committed and is now an invariant:
...
- deleting or drifting the lockfile must fail release validation rather than fall back to `npm install`.
...
- verification/build/test job: `contents: read` only;
- staging job: `contents: read` + `id-token: write`, and only after verification succeeds.
...
7. run rc.1 E2E against that exact `.tgz` without rebuilding/repacking;
8. re-verify its digest and upload the `.tgz` + release manifest as a GitHub Actions artifact;
9. if staged publishing is explicitly enabled for the repository/package, the separate OIDC job downloads and re-verifies that exact artifact, then stages it;
10. otherwise stop safely after producing the verified artifact.
...
For the public repository, and before the first npm tag:
...
- keep `SECURITY.md` as the sensitive vulnerability-reporting path.
...
- `package-lock.json` — committed deterministic dependency graph.
...
1. **Validation execution**
   ...
   - Accept only a conservative host-validated package-script command grammar: `npm|pnpm|yarn test` or `npm|pnpm|yarn|bun run <existing-script>`; no shell operators, inline code flags, redirections, command substitution, arbitrary binaries, or package-download execution. `bun test` is intentionally rejected because it invokes Bun's test runner rather than a package script.
...
5. `recovery.allowSafeResume` is enforced at the RPC boundary and reflected in UI.
...
7. `execution.roleTimeoutMs` is enforced for native/SDK mutating roles (Worker, Integrator and targeted fixes); the UI labels it as a mutation-role timeout rather than claiming it governs Reviewer requests.
8. All run-owned worktrees use `worktrees/<base-run-id>/<safe-lease-id>` so terminal cleanup removes retained worker and validation evidence correctly.
9. `@deepseek-ai/schemastery` is bundled into the server build instead of relying on transitive host composition; this avoids adding a new runtime install edge or lockfile churn.
...
- mutation-role timeout signals abort long-running mutating roles;
