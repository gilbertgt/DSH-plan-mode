...
export function Execution({ settings, patch }: any) {
  ...
  return <div className="planx-card"><h3>Execution</h3>
    ...
    <label>Mutation role timeout (ms) <input type="number" min={1000} max={3600000} value={e.roleTimeoutMs} onChange={x=>patch({roleTimeoutMs:Number(x.target.value)})}/></label>
    ...
    <p className="planx-muted">The timeout is enforced for native/SDK Worker, Integrator and targeted-fix mutation roles. Explicit ownership is always enforced and cannot be disabled.</p>
  </div>
}
