...
export type RepositoryPermission='admin'|'maintain'|'write'|'triage'|'read'|undefined
export async function ghRepositoryPermission(cwd:string,repository:string,login:string):Promise<RepositoryPermission>{
 ...
 try{
  ...
  return ['admin','maintain','write','triage','read'].includes(permission)?permission as RepositoryPermission:undefined
...
export async function trustedIssueTexts(
 cwd:string,
 repository:string,
 issue:any,
 permissionLookup:(cwd:string,repository:string,login:string)=>Promise<RepositoryPermission>=ghRepositoryPermission,
):Promise<string[]>{
 const sources=[
 ...
 for(const source of sources){
  ...
  if(allowed===undefined){allowed=canAuthorExternalControl(await permissionLookup(cwd,repository,source.login));cache.set(source.login,allowed)}
